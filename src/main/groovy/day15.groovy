...
class LensLibrary {
    ...
    LensLibrary(String input = "") {
        this.boxes = (0..255).collect { new Box() }

        if (input) {
            this.steps = input.split(",").collect { it.trim() }
        }
    }

    Integer hash(String str) {
        str.chars.inject(0) { acc, ch ->
            ((acc + ch) * 17) % 256
        }
    }
...
static void main(String[] args) {
    try {
        ...
        LensLibrary library = new LensLibrary(input)
